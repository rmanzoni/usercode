...
class PATreader() :
  ...
  def __init__(self, basic_histos, track_histos) :
  ...
  def looper(self, maxEvents=-1, pickSingleEvent=-1, pickSomeEvents=[], verbose=False) :
    ...
    for loopId, event in enumerate(self.events):
      ...
      if loopId%(self.events.size()/20)==0 : print '=====> \t', round( float(loopId)/float(totEvents)*100., 0) ,'%'
      
      if event.eventAuxiliary().event() not in pickSomeEvents and len(pickSomeEvents)>0 : continue
      if event.eventAuxiliary().event() != pickSingleEvent  and pickSingleEvent > 0     : continue
      ...
      if deltaR(off_tau[0], off_mu[0]) < 0.5               : continue  
      ...
      event.onl_tau_mu_vtx  = self.best_matching([event.tau], onlTausMuVtx , dR=0.5).values()[0]
      event.onl_tau_pix_vtx = self.best_matching([event.tau], onlTausPixVtx, dR=0.5).values()[0]
      
      event.tau.onlMu  = event.onl_tau_mu_vtx
      event.tau.onlPix = event.onl_tau_pix_vtx
      
      tau.genDM = self.genDecayMode(self.getGenJetConstituents(event.tau))
      ...
      self.fillBasicHistos(self.basic_histos,'offTaus',event.tau)

      if not event.onl_tau_mu_vtx :
        self.failinRecoHLT_mu  += 1
        passMu = False
        self.fillBasicHistos(self.basic_histos,'onlTauMu',event.tau)

      if not event.onl_tau_pix_vtx :
        self.failinRecoHLT_pix += 1  
        passPix = False
        self.fillBasicHistos(self.basic_histos,'onlTauPix',event.tau)

      if passMu :
        if event.onl_tau_mu_vtx.tauID('decayModeFinding') < 0.5  :
          self.failinDMHLT_mu    += 1
          passMu = False
          self.fillBasicHistos(self.basic_histos,'onlTauMuPassingDM',event.tau)

      if passPix :
        if event.onl_tau_pix_vtx.tauID('decayModeFinding') < 0.5 :
          self.failinDMHLT_pix   += 1
          passPix = False
          self.fillBasicHistos(self.basic_histos,'onlTauPixPassingDM',event.tau)

      if passMu :
        if event.onl_tau_mu_vtx.tauID('byIsolation') < 0.5 :
          self.failinIsoHLT_mu   += 1
          passMu = False
          self.fillBasicHistos(self.basic_histos,'onlTauMuPassingIso',event.tau)

      if passPix :
        if event.onl_tau_pix_vtx.tauID('byIsolation') < 0.5 :
          self.failinIsoHLT_pix  += 1        
          passPix = False
          self.fillBasicHistos(self.basic_histos,'onlTauPixPassingIso',event.tau)
  ...
  def fillTrackHistos(self, histos, name, particle, offvtx, hltpixvtx, hltmuvtx) :
    ...
    try    : histos[name]['dxy(offlineVtx)'           ] .Fill(track.dxy(offvtx)                              )
    except : pass
    try    : histos[name]['dz(offlineVtx)'            ] .Fill(track.dz(offvtx)                               )
    except : pass
    try    : histos[name]['dxy(hltPixVtx)'            ] .Fill(track.dxy(hltpixvtx)                           )
    except : pass
    try    : histos[name]['dz(hltPixVtx)'             ] .Fill(track.dz(hltpixvtx)                            )
    except : pass
    try    : histos[name]['dxy(hltMuVtx)'             ] .Fill(track.dxy(hltmuvtx)                            )
    except : pass
    try    : histos[name]['dz(hltMuVtx)'              ] .Fill(track.dz(hltmuvtx)                             )
    except : pass
